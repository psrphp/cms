...
class Date implements FieldInterface
{
    ...
    public static function onCreateFieldData()
    {
        Framework::execute(function (
            ...
            $db->query('ALTER TABLE <psrphp_cms_content_' . $model['name'] . '> ADD `' . $request->post('name') . '` date');
        });
    }
...
<div class="d-flex gap-1">
    <div>
        <input type="date" name="filter[{$field['name']}][min]" value="{$request->get('filter.'.$field['name'].'.min')}" class="form-control form-control-sm">
    </div>
    <div>
        <input type="date" name="filter[{$field['name']}][max]" value="{$request->get('filter.'.$field['name'].'.max')}" class="form-control form-control-sm">
    </div>
</div>
str;
