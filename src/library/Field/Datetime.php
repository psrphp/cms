...
class Datetime implements FieldInterface
{
    ...
    public static function onCreateFieldData()
    {
        Framework::execute(function (
            ...
            $db->query('ALTER TABLE <psrphp_cms_content_' . $model['name'] . '> ADD `' . $request->post('name') . '` datetime');
        });
    }
...
<div class="d-flex gap-1">
    <div>
        <input type="datetime-local" name="filter[{$field['name']}][min]" value="{$request->get('filter.'.$field['name'].'.min')}" class="form-control form-control-sm">
    </div>
    <div>
        <input type="datetime-local" name="filter[{$field['name']}][max]" value="{$request->get('filter.'.$field['name'].'.max')}" class="form-control form-control-sm">
    </div>
</div>
str;
